...
<<<<<<< HEAD
def cost2thresh3(cost,sub,c,b,lk,last,idc = [],costlist=[]):
    """A definition for loading the lookup table and finding the threshold associated with a particular cost for a particular subject in a particular block
    ...
    if len(th)>1:
        th=th[0] #if there are multiple thresholds, go down to the lower cost ####Is this right?!!!####
        print 'multiple thresh'
    elif len(th)<1: # if there are no thresholds for a specific cost...
        done = 1
        while done:
            idc = idc-1
            newcost = costlist[idc]
            print idc,newcost
            ind=np.where(lk[c][b][sub][1]==newcost)
            th=lk[c][b][sub][0][ind]
            if len(th) > 1:
                th = th[0]
                done = 0
...
>>>>>>> 91fe832914482a8b46a69f89ed1566f508d566cf
