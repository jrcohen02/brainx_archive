...
class GraphPartition(object):
    ...
    def apply_node_update(self, n, m1, m2, node_moved_mods, e_new, a_new):
        ...
        # If the source module is going to be left emtpy, this is
        # really a module merge operation, so we can call that routine instead
        #if len(node_moved_mods[0])==0:
        #    merged, e_new, a_new = self.compute_module_merge(m1, m2)[:3]
        #    self.apply_module_merge(m1, m2, merged, e_new, a_new)
        #    return m2
